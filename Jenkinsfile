        ...
        parallel(
          ...
          "Cypress": {
            node(label: 'docker') {
              script {
                try {
                  sh '''docker pull eeacms/eeacms/eea-website-backend; docker run --rm -d --name="$BUILD_TAG-plone" -e SITE="Plone" -e PROFILES="eea.progress.workflow:default" eeacms/eea-website-backend'''
