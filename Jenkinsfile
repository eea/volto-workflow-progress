pipeline {
  ...
  environment {
        ...
        NAMESPACE = "@eeacms"
        SONARQUBE_TAGS = "volto.eea.europa.eu,www.eea.europa.eu-ims,climate-energy.eea.europa.eu,demo-www.eea.europa.eu,prod-www.eea.europa.eu"
        DEPENDENCIES = ""
        ...
        parallel(
          ...
          "Cypress": {
            node(label: 'docker') {
              script {
                try {
                  sh '''docker pull eeacms/plone-backend; docker run --rm -d --name="$BUILD_TAG-plone" -e SITE="Plone" -e ADDONS="$ADDONS" -e PROFILES="$PROFILES" eeacms/plone-backend'''
